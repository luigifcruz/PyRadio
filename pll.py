...
class PLL:
    def __init__(self, fs, length):
        ...
        signal = self.sinx(self.fs/4, 1.0, self.times)
    ...
    @staticmethod
    @njit(fastmath=True)
    def sinx(freq, phase, times):
        return np.sin(2.0*np.pi*freq*times+phase)
    ...
    def step(self, x):
        ...
        self.phi = np.argmax(error_ls)
        
    def mult(self, mult=1):
        omega = ((self.phi*mult)+(np.pi/mult))
        return self.sinx(self.freq*mult, omega, self.times)